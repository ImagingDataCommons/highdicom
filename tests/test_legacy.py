...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data = {
            ...
            "DA": [
                ...
                DataElement(524320, "DA", "19950809"),
                DataElement(524320, "DA", "19950810"),
            ],
            "TM": [
                # Keyword: (0008, 0030) StudyTime
                DataElement(524336, "TM", "100044"),
                DataElement(524336, "TM", "100044"),
                DataElement(524336, "TM", "100054"),
            ],
            "US": [
    ...
    def test_attribute_equality(self) -> None:
        for [v1, v2, v3] in self.data.values():
            assert sop._isequal(v1.value, v2.value)
            assert not sop._isequal(v1.value, v3.value)
        ...
        for j, m in enumerate(self._modalities):
            with self.subTest(m=m):
                ...
                ref_dataset_seq = getattr(
                    self,
                    f"_ref_dataset_seq_{self._modalities[next_idx]}"
                )
                with self.assertRaises(ValueError):
                    LegacyConverterClass(
                        ref_dataset_seq,
