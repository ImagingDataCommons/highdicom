...
class TestFrameSetCollection(unittest.TestCase):
    ...
    def test_frameset_detection(self) -> None:
        ...
        for i in range(1, 10):
            ...
            assert len(fset_collection.FrameSets) == i
    ...
    def test_frameset_framecount_detection(self) -> None:
        for i in range(1, 10):
            ...
            fset_collection = sop.FrameSetCollection(data)
            assert len(fset_collection.FrameSets) == 1
            assert len(fset_collection.FrameSets[0].Frames) == i
    ...
    def test_conversion(self) -> None:
        for i in range(1, 10):
            for j in range(3):
                ...
                assert len(fset_collection.FrameSets) == 1
                assert len(fset_collection.FrameSets[0].Frames) == i
                convertor = sop.LegacyConvertedEnhanceImage(
                    fset_collection.FrameSets[0],
                    generate_uid(),
                    555,
                    generate_uid(),
                    111)
                convertor.convert2mf()
