...
@pytest.mark.parametrize(
    ...
    [
        ...
        Volume.from_attributes(
            array=np.random.randint(0, 100, (16, 16, 16)),
            image_position=[0.0, 0.0, 0.0],
            image_orientation=[-0.9267662161157189, 0.32283606313442387, -0.1920449348627007, -0.3751482085550474, -0.7693372329674889, 0.5170919101937937]
,
            pixel_spacing=[1.0, 1.0],
            spacing_between_slices=1.0,
            coordinate_system='PATIENT'
        ...
        Volume.from_attributes(
            array=np.random.randint(0, 100, (16, 16, 16)),
            image_position=[0.0, 0.0, 0.0],
            image_orientation=[-0.16411694786392106, -0.7887835415036736, 0.5923564400567902, 0.9859501024515502, -0.11222667947664411, 0.12372375636644917],
            pixel_spacing=[1.0, 1.0],
            spacing_between_slices=1.0,
            coordinate_system='PATIENT'
        ),
        Volume.from_attributes(
            array=np.random.randint(0, 100, (16, 16, 16)),
            image_position=[0.0, 0.0, 0.0],
            image_orientation=[-0.4161787354389772, 0.440351232633788, 0.7955413578729377, -0.2771687765466385, -0.8947097563255662, 0.350245515665062],
            pixel_spacing=[1.0, 1.0],
            spacing_between_slices=1.0,
            coordinate_system='PATIENT'
        ),
        ...
        Volume.from_attributes(
            ...
            image_orientation=[-0.3056572521325831, -0.9434667295206645, -0.12823484123411946, 0.9440777770580763, -0.3177984972478506, 0.08787073467366081],
        ...
        Volume.from_attributes(
            ...
            image_orientation=[-0.7034764816836532, 0.05445328347346446, -0.7086294374614612, -0.24703727011677554, -0.9536256538386763, 0.1719613314498601],
        ...
        Volume.from_attributes(
            ...
            image_orientation=[-0.7374895773326877, 0.5716640328411087, 0.35959610242811746, -0.35709313761636013, -0.7820074118766409, 0.510831575802926],
...
def test_roundtrip(vol: Volume):
    ...
    # assert np.allclose(vol.direction, itk_im.GetDirection(), atol=1e-4)
    assert np.allclose(vol.direction.flatten(), sitk_im.GetDirection(), atol=1e-4)

    # assert (vol.array == itk.GetArrayFromImage(itk_im).transpose(2, 1, 0)).all()
    assert (vol.array == sitk.GetArrayFromImage(sitk_im).transpose(2, 1, 0)).all()
...
def test_dtype_sitk(dtype: np.dtype):
    ...
    if dtype == np.bool_:
        ...
        with pytest.warns(
            UserWarning,
            match='SimpleITK does not support float16 data. Safely recasting to float32.'
        ):
            sitk_roundtrip = Volume.from_sitk(volume.to_sitk())
        ...
        with pytest.warns(
            UserWarning,
            match='SimpleITK does not support float128 data. Safely recasting to float64.'
        ):
            sitk_roundtrip = Volume.from_sitk(volume.to_sitk())
        ...
        with pytest.raises(
            ValueError,
            match='SimpleITK does not support float128 data. Safely recasting to float64 is not possible.'
        ):
            sitk_roundtrip = Volume.from_sitk(volume.to_sitk())
...
#         with pytest.warns(
#             UserWarning,
#             match='ITK does not support int8 data. Safely recasting to int16.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
...
#         with pytest.warns(
#             UserWarning,
#             match='ITK does not support int64 data. Safely recasting to int32.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
...
#         with pytest.raises(
#             ValueError,
#             match='ITK does not support int64 data. Safely recasting to int32 is not possible.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
#
...
#         with pytest.warns(
#             UserWarning,
#             match='ITK does not support float16 data. Safely recasting to float32.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
...
#         with pytest.warns(
#             UserWarning,
#             match='ITK does not support float128 data. Safely recasting to float64.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
...
#         with pytest.raises(
#             ValueError,
#             match='ITK does not support float128 data. Safely recasting to float64 is not possible.'
#         ):
#             itk_roundtrip = Volume.from_itk(volume.to_itk())
#
#         volume.array[(0, 0, 0)] = 1.1 * np.float128(np.finfo(np.float64).min)
...
@pytest.mark.parametrize(
    ...
    [
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/2_t1_mp2rage_sag_p3_32.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_2_t1_mp2rage_sag_p3_32_INV1.nii.gz'
        ),
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/3_t1_mp2rage_sag_p3_32.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_3_t1_mp2rage_sag_p3_32_INV2.nii.gz'
        ),
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/4_t1_mp2rage_sag_p3_32.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_4_t1_mp2rage_sag_p3_32_UNI_Images.nii.gz'
        ),
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/5_HCP_T1.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_5_HCP_T1.nii.gz'
        ),
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/6_T1_mprage_ns_sag_p2.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_6_T1_mprage_ns_sag_p2.nii.gz'
        ),
        (
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/In/8_T1_memprage_rms.zip',
            'https://github.com/neurolabusc/dcm_qa_mprage/raw/refs/heads/main/Ref/Si_8_T1_memprage_rms_RMS.nii.gz'
        ),
    ]
)
def test_nifti_equivalence_zip(zip_url: str, nifti_url: str):
    ...
    # orientation = get_closest_patient_orientation(np.reshape(itk_im.GetDirection(), (3, 3)))
    orientation = get_closest_patient_orientation(np.reshape(sitk_im.GetDirection(), (3, 3)))
    ...
    # assert np.allclose(oriented_vol.direction, itk_im.GetDirection(), atol=1e-4)
    assert np.allclose(oriented_vol.direction.flatten(), sitk_im.GetDirection(), atol=1e-4)

    # assert (oriented_vol.array == itk.GetArrayFromImage(itk_im).transpose(2, 1, 0)).all()
    assert (oriented_vol.array == sitk.GetArrayFromImage(sitk_im).transpose(2, 1, 0)).all()
...
@pytest.mark.parametrize(
    ...
    [
        (
            [
                f'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/In/2_me_FieldMap_GRE/{i:04d}.dcm'
                for i in range(1, 37)
            ],
            'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/Ref/me_FieldMap_GRE_2_e1.nii'
        ),
        (
            [
                f'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/In/2_me_FieldMap_GRE/{i:04d}_e2.dcm'
                for i in range(1, 37)
            ],
            'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/Ref/me_FieldMap_GRE_2_e2.nii'
        ),
        (
            [
                f'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/In/3_me_FieldMap_GRE/{i:04d}_e2_ph.dcm'
                for i in range(1, 37)
            ],
            'https://github.com/neurolabusc/dcm_qa_me/raw/refs/heads/master/Ref/me_FieldMap_GRE_3_e2_ph.nii'
        ),
        (
            [
                f'https://github.com/neurolabusc/dcm_qa_pdt2/raw/refs/heads/main/In/Siemens/VE11/{i:04d}.dcm'
                for i in range(1, 36)
            ],
            'https://github.com/neurolabusc/dcm_qa_pdt2/raw/refs/heads/main/Ref/Siemens_pd+t2_tse_sag_ISO_1.8mm_3_e1.nii'
        ),
        (
            [
                f'https://github.com/neurolabusc/dcm_qa_pdt2/raw/refs/heads/main/In/Siemens/VE11/{i:04d}_e2.dcm'
                for i in range(36, 71)
            ],
            'https://github.com/neurolabusc/dcm_qa_pdt2/raw/refs/heads/main/Ref/Siemens_pd+t2_tse_sag_ISO_1.8mm_3_e2.nii'
        ),
    ]
)
