...
Parametric Maps share many similarities with Segmentations (see :ref:`seg`).
Both are multi-frame DICOM objects storing pixel arrays that are derived from
other images. The multiple frames in a both objects can store slices of a
...
applications), among other arrangements. The crucial difference is that
Segmentations store a discrete classification in each pixel (though the
classification may be probabilistic), whereas parametric maps store arrays of
continuous measurements.

An important advantage of Parametric maps over all other DICOM objects that
store pixel data is that they can store pixels in floating point format using
either 32-bit (single precision) or 64-bit (double precision) floats. Signed
and unsigned integer pixel values are also supported. They also include
information describing the physical quantity that pixels image represents.
...
different measurements can be included in one Parametric Map objects.
...
"real world" values and specify the semantics of those real world values
after mapping.

In highdicom, *real-world value mappings* are represented by the
...
- ``slope`` and ``intercept`` (``int`` or ``float``): These parameters may be
  used specify a linear relationship between the stored values and the
  real-world values of the form ``real_world_value = stored_value * slope +
  intercept``.
- ``lut_data`` (``Sequence[float]``): As an alternative to ``slope`` and
  ``intercept``, ``lut_data`` contains a sequence of values to serve as a
  lookup table for mapping stored values into real-world values in case of a
  non-linear relationship. The sequence should contain one entry for each value
  in the specified ``value_range`` such that ``len(lut_data) == value_range[1]
  value_range[0] + 1``. For example, in case of a value range of ``(0,
  255)``, the sequence shall have ``256`` entries - one for each value in the
  given range.

If neither ``lut_data`` nor ``slope`` and ``intercept`` is specified, a linear
relationship is assumed with a default intercept of 0 and slope of 1, (i.e. an
identity mapping).

Additionally, the following parameters are used to describe the mapping and
...
The following snippet constructs two basic Real World Value Mappings that each
use a linear relationship between stored values and real-world values:
...
the :class:`highdicom.pm.ParametricMap` as a ``numpy.ndarray`` via the
``pixel_array`` parameter. There are several possible variations here.
...
- A 3D array, giving multiple 2D parametric map frames derived from either a
  series of input image or of a single multiframe image. Frames are stacked
  down the first dimension (index 0)
