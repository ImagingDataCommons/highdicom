...
group contains measurements and/or evaluations about a particular image or
...
- :class:`highdicom.sr.PlanarROIMeasurementsAndQualitativeEvaluations`: Refers
  to a 2D region of an image defined in image coordinates.
- :class:`highdicom.sr.VolumetricROIMeasurementsAndQualitativeEvaluations`:
  Refers to a 3D region within the frame of reference of an image or image
  series.

A single Structured Report may contain a mixture of Measurement Groups of these
different types in any combination (as long as there is at least one group).
...
1. The image region is specified directly in the SR using a
   ...
   defining the SR are stored within the measurement group itself. The choice
   between :class:`highdicom.sr.ImageRegion` and
...
and evaluations themselves. Information derived from the images or image
